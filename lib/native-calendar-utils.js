    ...
    addOpen(data) {
        try {
            if (data.status === Status.OPEN) {
                ...
                throw new Error(`Addition of ${Status.OPEN} status is possible with data with property status ${Status.OPEN} only.`);
            }
        }
        catch (error) {
            return error;
    ...
    addBooked(data) {
        try {
            if (data.status === Status.BOOKED) {
                ...
                throw new Error(`Addition of ${Status.BOOKED} status is possible with data with property status ${Status.BOOKED} only.`);
            }
        }
        catch (error) {
            return error;
    ...
    addBlocked(data) {
        try {
            if (data.status === Status.BLOCKED) {
                ...
                throw new Error(`Addition of ${Status.BLOCKED} status is possible with data with property status ${Status.BLOCKED} only.`);
            }
        }
        catch (error) {
            return error;
...
exports.default = NativeCalendarUtils;
//mock data
// const data = [
//   {
//     _id: "1234560",
